...
- **桌面模式** — Tauri v2 桌面应用，内嵌 Python 后端 sidecar，一键启动
...
- **`frontend/src-tauri/`** — Tauri v2 桌面应用，通过 shell plugin 管理 Python 后端 sidecar
- 桌面模式下前端通过 `window.__TAURI__` 检测环境，直连 `http://localhost:8000`
